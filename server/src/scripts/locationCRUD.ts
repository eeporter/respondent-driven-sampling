...
async function importLocations(filePath: string): Promise<void> {
	...
	for (let i = 0; i < locations.length; i++) {
		...
		try {
			...
			console.log(
				`  ✓ [${index}/${locations.length}] Created: ${locationData.hubName}`
			);
		} catch (error) {
			const errorMessage =
...
async function getLocation(identifier: string): Promise<void> {
	...
	try {
		...
		switch (operation) {
			case 'create':
				...
				await createLocation(args[1], args[2], args[3], args[4]);
				break;

			case 'import':
				if (args.length < 2) {
					console.error(
						'Error: import requires 1 argument: <yamlFilePath>'
					);
					console.error(
						'  Example: npm run location -- import locations.yaml'
					);
			...
			case 'update': {
				...
				await updateLocation(identifier, updates);
				break;
			}
